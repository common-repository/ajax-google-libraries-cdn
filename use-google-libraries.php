<?php
/*
  Plugin Name: Ajax Google Libraries CDN
  Plugin URI: http://onlinewebapplication.com/ajax-google-libraries-cdn
  Description:Allows your site to use common javascript libraries from Google's AJAX Libraries CDN, rather than from own local copies.
  Author: Pankaj Jha
  Version: 1.0.2
...
if ( !class_exists( 'Class_AjaxGoogleLibrariesCDN' ) ) {
	...
	class Class_AjaxGoogleLibrariesCDN {

		private static $instance;

		public static function get_instance() {
			if ( !isset( self::$instance ) ) {
				self::$instance =  new Class_AjaxGoogleLibrariesCDN();
			}
		...
		'<strong>Ajax Google Libraries CDN</strong>: Another plugin has registered or enqued a script before the "init" action.  Attempting to work around it.';
		/**
		 * PHP 4 Compatible Constructor
		 */
		function Class_AjaxGoogleLibrariesCDN() {$this->__construct();}
		...
		static function configure_plugin() {
			add_action( 'wp_default_scripts',
				array( 'Class_AjaxGoogleLibrariesCDN',
					'replace_default_scripts_action' ),
				1000 );
			add_filter( 'script_loader_src',
				array( "Class_AjaxGoogleLibrariesCDN", "remove_ver_query_filter" ),
				1000 );
			add_filter( 'init', array( "Class_AjaxGoogleLibrariesCDN", "setup_filter" ) );
		...
		function replace_default_scripts( &$scripts ) {
			...
			foreach ( $this->google_scripts as $name => $values ) {
				if ( $script = $scripts->query( $name ) ) {
					...
					if ( strpos( $ver, '-' ) !== false ) {
						if ( WP_DEBUG !== false ) {
							error_log("WordPress appears to be using a non-standard version of $name (version $ver). Ajax Google Libraries CDN not enabled for $name.");
						}
						continue;
					}
...
if ( class_exists( 'Class_AjaxGoogleLibrariesCDN' ) ) {
	Class_AjaxGoogleLibrariesCDN::configure_plugin();
}
